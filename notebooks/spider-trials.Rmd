  ...
  visit_node <- function(account) {
    ...
    try({
      ...
      if (nrow(user) > 0) {
        ...
        dbAppentTable(con, 'node_list', tibble(handle = handle, visited = "True"))
      }
...
db_location = paste0(lubridate::today(), "-", seed_handle, ".sqlite")
```
...
g <- as_tbl_graph(loc_edges) %>% 
  convert(to_simple)
g %>%
  mutate(cent = centrality_degree(mode = "in")) %>%
  ggraph(weights = "") +
  geom_edge_link0(alpha = .2) +
  geom_node_point(aes(size = cent)) +
  geom_node_label(aes(label = name), data = ~slice_max(.x, cent, n = 15), repel = T)
```
